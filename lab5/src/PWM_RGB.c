...
void updateTTC0(float red, float green, float blue);
...
int pwmInit(void)
{
	...
	//Initialize XADC driver and keep reading external voltage on AUX14
	Status = readXADCInput(SYSMON_DEVICE_ID);
	if (Status != XST_SUCCESS) {
...
void generateFullRGB(float vin) {
	...
	//Assign Initial values of the RGB
	float red = 1;
	float blue = 0;
	float green = 0;
	float scaledVin;

	//0-1 volts are divided into 6 parts i.e (1/6 = 0.166)
	float sampling_interval = 0.166;

	//0-0.166 interval is scaled to 0-1 interval with this scaling factor i.e. (1/0.166 = 6.024)
	scaledVin = 6.024 * vin;

	// voltage 0-0.166
	if (vin < sampling_interval) {

		red = 1, green = scaledVin, blue = 0;
	}

	// voltage 0.166 - 0.33
	else if (vin < 2 * sampling_interval) {

		red = 2 - scaledVin, green = 1, blue = 0;
	}

	// voltage 0.33 - 0.5
	else if (vin < 3 * sampling_interval) {

		red = 0, green = 1, blue = scaledVin - 2;
	}

	// voltage 0.5 - 0.66
	else if (vin < 4 * sampling_interval) {

		red = 0, green = 4 - scaledVin, blue = 1;
	}

	// voltage 0.66 - 0.83
	else if (vin < 5 * sampling_interval) {

		red = scaledVin - 4, green = 0, blue = 1;
	}

	// voltage 0.83 - 1.00
	else if (vin < 6 * sampling_interval) {

		red = 1, green = 0, blue = 6 - scaledVin;
	}

	updateTTC0(red, green, blue);
...
void updateTTC0(float red, float green, float blue) {
	...
	float maxTimerCount = 65535;

	XTtcPs_WriteReg(XPAR_XTTCPS_0_BASEADDR, XTTCPS_MATCH_0_OFFSET,
			(red * maxTimerCount));

	XTtcPs_WriteReg(XPAR_XTTCPS_1_BASEADDR, XTTCPS_MATCH_0_OFFSET,
			(green * maxTimerCount));

	XTtcPs_WriteReg(XPAR_XTTCPS_2_BASEADDR, XTTCPS_MATCH_0_OFFSET,
			(blue * maxTimerCount));
}
